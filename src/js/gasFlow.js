...
// 处理返回结果，拼凑字段
export const formatterCurveData = (data) => {
  const xData = []
  const yDatas = []
  ...
  for (var k in firstData) {
    dict.forEach(ele => {
      if (ele.param == k) {
        ...
        for (var key in data) {
          arr.push({
            value: data[key][k] ? data[key][k].mult : '',
...
export const addNewSockData = (diagramDataObj, data) => {
  const { series, legend, x } = diagramDataObj
      ...
      for (var k in data[key]) {
        if (ele.nameCode == k) {
          if (new Date(key).getMinutes() == 0 || new Date(key).getTime() > new Date(x[59]).getTime()) {
            ...
            ele.data.forEach(item => {
              if (item.time == key) {
                item.point = data[key][k].val
                item.value = data[key][k].mult,
                item.k = k
              }
            })
          }
