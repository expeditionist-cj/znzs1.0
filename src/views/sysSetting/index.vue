<template>
  <div class="sysSetting">
    <div class="IntelligentModel-left">
      <!-- 左侧纵向导航栏 -->
      <ul class="ul-outside">
        <li class="li-outside" v-for="item in menuData" :key="item.id">
          ...
          <ul class="ul-inside">
            ...
            >
              <div
                class="li-inside-div"
                @click="onCheck(itm)"
                :class="{ active: itm.mname == activeSysSetting }"
                :style="{
...
export default {
  ...
  computed: {
    ...mapState({
      activeSysSetting: (state) => state.app.activeSysSetting,
      menu: (state) => state.user.menu,
    }),
  },
  created() {
    window.$ = $; 
    let menuData = this.menu.filter((item) => {
      return item.mname == "后台";
    }); 
    menuData = JSON.parse(JSON.stringify(menuData[0]));  
