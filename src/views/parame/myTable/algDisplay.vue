<template>
  <div class="myTable-wrap">
    <div class="main-title" v-if="pointTitle">{{ pointTitle }}</div>
    <el-table show-header :data="tableData" size="small" border style="width: 100%">
      <el-table-column
        prop="param_cn"
        label="参数名称"
        width="180"
      ...
      <el-table-column show prop="name" label="类型" width="180" align="center">
        <template slot-scope="scope">
          ...
          <span v-else>{{scope.row.is_bind == 1?'绑定':'设定'}}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="single_point"
